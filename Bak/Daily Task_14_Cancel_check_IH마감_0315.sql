...
LEFT JOIN
	(SELECT 회원번호, 변경항목
	FROM MRMRT.그린피스동아시아서울사무소0868.dbo.UV_GP_변경이력
	WHERE 변경항목 IN ('계좌번호', '은행', '납부방법', '증빙자료 추가')
    AND CONVERT(DATE,변경일시) >= CONVERT(DATE,GETDATE()-3)
  ) CH
ON H.회원번호 = CH.회원번호
LEFT JOIN
	(SELECT 회원번호, 신청일
	FROM MRMRT.그린피스동아시아서울사무소0868.dbo.UV_GP_CMS승인정보
	WHERE 신청일 >= CONVERT(DATE,GETDATE()-3)
	UNION ALL
	SELECT 회원번호, 승인일
	FROM MRMRT.그린피스동아시아서울사무소0868.dbo.UV_GP_신용카드승인정보
	WHERE 승인일 >= CONVERT(DATE,GETDATE()-3) 
	) BCA
ON H.회원번호  = BCA.회원번호
...
WHERE 기록분류상세 = '자발_Unfreezing'
	...
	AND (D.회원번호 IS NULL
	OR 
		(CH.회원번호 IS NULL AND BCA.회원번호 IS NULL) )
