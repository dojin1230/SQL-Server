...
SELECT
	...
	WHERE Campaign_Date = CONVERT(DATE,GETDATE()-5) ) AS RAW_D5_AGO,
	(SELECT COUNT(*)
	FROM [work].[dbo].[en_data]
	WHERE Campaign_Date = CONVERT(DATE,GETDATE()-4) ) AS RAW_D4_AGO,
	(SELECT COUNT(*)
	FROM [work].[dbo].[en_data]
	WHERE Campaign_Date = CONVERT(DATE,GETDATE()-3) ) AS RAW_D3_AGO,
	(SELECT COUNT(*)
	FROM [work].[dbo].[en_data]
	WHERE Campaign_Date = CONVERT(DATE,GETDATE()-2) ) AS RAW_D2_AGO,
	(SELECT COUNT(*)
	FROM [work].[dbo].[en_data]
	WHERE Campaign_Date = CONVERT(DATE,GETDATE()-1) ) AS RAW_D1_AGO,
	(SELECT COUNT(*)
	FROM #001) AS C1,
	(SELECT COUNT(*)
	FROM #001_1) AS C2,
	(SELECT COUNT(*)
	FROM [work].[dbo].[petition_all_90]) AS C7,
	(SELECT COUNT(*)
	FROM #003_1) AS C8,
	(SELECT COUNT(*)
	FROM #003_2) AS C9,
	(SELECT COUNT(*)
	FROM [work].[dbo].[db0_clnt_i_phone_mail]) AS C10,
	(SELECT COUNT(*)
	FROM #004) AS C11,
	(SELECT COUNT(*)
	FROM [work].[dbo].[petition_daily_before]) AS C12,
	(SELECT COUNT(*)
	FROM [work].[dbo].[petition_daily]) AS C13
