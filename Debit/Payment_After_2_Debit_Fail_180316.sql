...
FROM
	...
	FROM 
		MRMRT.그린피스동아시아서울사무소0868.DBO.UV_GP_신용카드결제결과
				WHERE
		출금일 >= DATEADD(day, -3, GETDATE())
		and 결과 ='실패' AND 실패사유  NOT LIKE '%잔액%' AND 실패사유 NOT LIKE '%잔고%' AND 실패사유 NOT LIKE '%한도%' AND 실패사유 NOT LIKE '%포인트%'
	) BCR
LEFT JOIN
...
WHERE
	...
	AND D45.회원번호 is null				-- 최근 45일 이내에 전화를 받은 사람 제외
	AND DN.회원번호 is null				-- 통화반응이 1번인 사람 제외
