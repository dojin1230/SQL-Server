...
LEFT JOIN
	...
	WHERE
		기록분류 like 'TM_후원증액'
		AND 처리진행사항 in ('SK-진행', 'IH-지연', 'IH-진행')) UP
ON
	D.회원번호 = UP.회원번호
LEFT JOIN
	dbo.no_response NR
...
ON D.회원번호 = ING.회원번호
WHERE
	D.가입일 >= DATEADD(mm, DATEDIFF(mm, 0, GETDATE())-7, 0)
	AND D.가입일 < DATEADD(mm, DATEDIFF(mm, 0, GETDATE())-6, 0)	-- 가입일 6개월차만 
	...
	AND UP.회원번호 is null										-- 후원증액 1년차 콜과 겹치지 않도록
