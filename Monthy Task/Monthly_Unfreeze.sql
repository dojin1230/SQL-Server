...
LEFT JOIN
	...
	WHERE
		기록분류 like 'TM_후원재개%'
		AND 기록분류상세 not in ('제외', '무응')
		AND 참고일 > CONVERT(varchar(10), DATEADD(month, -6, GETDATE()), 126)) UF6
ON
	F.회원번호 = UF6.회원번호
LEFT JOIN
	(SELECT
...
WHERE
	...
	AND UF6.회원번호 is null																			-- 최근 6개월 이내에 후원재개콜을 받은 적 있는 사람 제외
	AND UFY.회원번호 is null																			-- 후원재개콜을 받고 동의한 적 있는 사람 제외
	AND UN.회원번호 is null																			-- 결번 제외
	AND CRE.회원번호 is null																			-- 통화반응이 1번인 사람 제외
	AND C.회원번호 is null																			-- 취소 지연 중인 사람 제외
	AND NR.회원번호 is null																			-- 연속 무응 3번인 사람 제외
