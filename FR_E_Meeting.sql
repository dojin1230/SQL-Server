...
SELECT 
	...
	@YEAR_DONOR = sum(case when 가입일 >= CONVERT(varchar(10), '2018-01-01', 126) then 1 else 0 end),
	@YEAR_NETDONOR = sum(case when 가입일 >= CONVERT(varchar(10), '2018-01-01', 126) and 총납부금액 > 1 then 1 else 0 end),
	@MONTH_DONOR = sum(case when 가입일 >= CONVERT(varchar(10), DATEADD(mm, DATEDIFF(mm, 0, GETDATE()), 0), 126) then 1 else 0 end),
	@DDC_DONOR = sum(case when 가입경로='거리모집' and 가입일 >= CONVERT(varchar(10), '2018-01-01', 126) then 1 else 0 end),
	@DDC_NETDONOR = sum(case when 가입경로='거리모집' and 가입일 >= CONVERT(varchar(10), '2018-01-01', 126) and 최초납부년월 is not null then 1 else 0 end),
	@WEB_DONOR = sum(case when 가입경로='인터넷/홈페이지' and 가입일 >= CONVERT(varchar(10), '2018-01-01', 126) then 1 else 0 end),
	@WEB_NETDONOR = sum(case when 가입경로='인터넷/홈페이지' and 가입일 >= CONVERT(varchar(10), '2018-01-01', 126) and 최초납부년월 is not null then 1 else 0 end),
	@TM_DONOR = sum(case when 가입경로='Lead Conversion' and 가입일 >= CONVERT(varchar(10), '2018-01-01', 126) then 1 else 0 end),
	@TM_NETDONOR = sum(case when 가입경로='Lead Conversion' and 가입일 >= CONVERT(varchar(10), '2018-01-01', 126) and 최초납부년월 is not null then 1 else 0 end)
FROM 
...
PRINT 
	...
	'Total # of dnr/net(2018) : ',  @YEAR_DONOR, '/', @YEAR_NETDONOR,  ' (', @YEAR_NETDONOR_RATIO, '%)', CHAR(13), CHAR(13), 
